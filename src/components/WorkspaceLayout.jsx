...
export default function WorkspaceLayout({ children }) {
  ...
  return (
    <div className="h-screen bg-slate-950 text-slate-100 flex flex-col font-sans antialiased overflow-hidden">
      ...
      <div className="md:hidden bg-slate-900/90 border-b border-slate-800 px-3 py-2 flex items-center justify-between z-30 shrink-0">
        <div className="relative flex-1 mr-2" ref={dropdownRef}>
          <button
            onClick={() => setDropdownOpen(!dropdownOpen)}
            className="w-full px-2.5 py-1.5 rounded-md bg-slate-950/80 border border-slate-800 flex items-center justify-between text-left transition-all"
          >
            <div className="flex items-center gap-2 min-w-0">
              <div className="w-5 h-5 rounded bg-indigo-600/20 text-indigo-400 flex items-center justify-center font-bold text-[10px] shrink-0">
          ...
          {dropdownOpen && (
            <div className="absolute top-full left-0 right-0 mt-1 rounded-md bg-slate-900 border border-slate-800 shadow-2xl py-1.5 z-50">
                ...
                {workspaces.map((ws) => (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-2.5 py-1.5 rounded text-xs text-left ${
                      currentWs?.id === ws.id ? 'bg-indigo-600/20 text-indigo-400 font-medium' : 'text-slate-300 hover:bg-slate-800'
                    }`}
                  >
                    <span className="truncate">{ws.name}</span>
                    {currentWs?.id === ws.id && <Check className="w-3.5 h-3.5 text-indigo-400" />}
                  </button>
                ))}
              </div>
              <div className="border-t border-slate-800 mt-1.5 pt-1.5 px-1">
                <button
                  onClick={() => {
                    setDropdownOpen(false)
                    setCreateModalOpen(true)
                  }}
                  className="w-full flex items-center gap-1.5 px-2.5 py-1.5 rounded text-xs text-indigo-400 hover:bg-indigo-600/10 font-medium"
                >
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="md:hidden fixed bottom-0 left-0 right-0 bg-slate-900/95 backdrop-blur-md border-t border-slate-800 z-40 flex items-center justify-between px-2 py-1.5 overflow-x-auto">
          {navItems.map((item) => (
            <NavLink
              key={item.label}
              to={item.path}
              className={({ isActive }) =>
                `flex flex-col items-center gap-0.5 px-2 py-1 rounded-md text-[10px] min-w-[50px] shrink-0 transition-colors ${
                  isActive ? 'text-indigo-400 font-semibold bg-slate-800/80' : 'text-slate-400 hover:text-slate-200'
                }`
              }
            >
              <item.icon className="w-4 h-4" />
