  ...
  return (
    <WorkspaceLayout>
      <div className="h-[calc(100vh-10.5rem)] md:h-[calc(100vh-6.5rem)] flex flex-col md:flex-row gap-3 sm:gap-4 overflow-hidden">
        ...
        <div className="w-full md:w-64 max-h-32 md:max-h-none bg-white dark:bg-slate-900/70 border border-slate-200 dark:border-slate-800 rounded-lg p-2.5 sm:p-3.5 flex flex-col shrink-0 shadow-sm">
          <div className="flex items-center justify-between mb-2 sm:mb-3 px-1">
            ...
            <button
              ...
              className="px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-md bg-indigo-50 dark:bg-indigo-600/20 hover:bg-indigo-600 text-indigo-600 dark:text-indigo-400 hover:text-white dark:hover:text-white transition-all text-xs flex items-center gap-1 font-medium border border-indigo-200 dark:border-indigo-500/20"
              title="New Chat"
            >
              ...
              return (
                ...
                >
                  ...
                  <button
                    ...
                    className="opacity-0 group-hover:opacity-100 p-1 text-slate-400 hover:text-red-600 dark:hover:text-red-400 transition-opacity shrink-0"
                    title="Delete Chat"
                  >
        ...
        <div className="flex-1 bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-lg flex flex-col overflow-hidden shadow-sm dark:shadow-xl">
          {/* Header Banner */}
          <div className="px-5 py-3 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/60 flex items-center justify-between">
            ...
            <div className="text-[11px] text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
              <BookOpen className="w-3.5 h-3.5 text-indigo-600 dark:text-indigo-400" /> Hybrid Search Enabled
            </div>
          </div>

          {/* Error Banner */}
          {error && (
            <div className="m-4 p-3.5 rounded-md bg-red-500/10 border border-red-500/30 text-red-600 dark:text-red-400 text-xs flex items-start gap-2.5">
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 bg-slate-50/50 dark:bg-transparent">
            {messages.length === 0 && !loading && (
              <div className="h-full flex flex-col items-center justify-center text-center space-y-4 max-w-md mx-auto my-auto text-slate-500 dark:text-slate-400">
                <div className="w-11 h-11 rounded-md bg-indigo-50 dark:bg-indigo-600/15 border border-indigo-200 dark:border-indigo-500/20 text-indigo-600 dark:text-indigo-400 flex items-center justify-center">
                  <Sparkles className="w-5 h-5" />
                </div>
                <div className="space-y-1.5">
                  <h3 className="text-sm font-semibold text-slate-900 dark:text-white">Ask your workspace documents</h3>
                ...
                >
                  <div
                    className={`max-w-3xl rounded-md p-4 text-xs sm:text-sm leading-relaxed shadow-sm ${isUser
                      ...
                      : 'bg-white dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/60 text-slate-800 dark:text-slate-100'
                      }`}
                  >
            ...
            {loading && (
              <div className="flex items-center gap-3 text-xs text-indigo-600 dark:text-indigo-400 bg-white dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 p-3 rounded-md w-fit shadow-sm animate-pulse">
                <div className="w-4 h-4 rounded-md border-2 border-indigo-600 dark:border-indigo-400 border-t-transparent animate-spin" />
          ...
          {/* Input Box Form */}
          <form onSubmit={handleSendQuery} className="p-4 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950/80 space-y-2">
            {inputQuery.length > 4000 && (
              <div className="text-xs text-red-600 dark:text-red-400 font-medium flex items-center gap-1.5 animate-in fade-in duration-150">
            ...
            <div className="flex items-center gap-3">
              <div className="relative flex-1">
                <input
                  ...
                  className={`w-full px-4 py-2.5 pr-20 rounded-md bg-slate-50 dark:bg-slate-900 border ${inputQuery.length > 4000 ? 'border-red-500 text-red-600 dark:text-red-200' : 'border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white'
                    } placeholder-slate-400 dark:placeholder-slate-500 text-xs sm:text-sm focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 disabled:opacity-50 transition-all`}
                />
              </div>
              <button
                type="submit"
